...
micro:bit는 한 줄 CSV 데이터를 보내고, 웹앱은 설정 파일의 필드 순서에 맞춰 값을 해석한다. 이후 상태 저장소에 샘플을 쌓고, 카드·차트·로그·CSV 다운로드가 같은 데이터를 사용한다.

## 설정 계층

- 주요 파일: `js/sensor-config.js`
- 역할: 앱 제목, 시작 명령, 샘플 주기, CSV 파일명, 필드 목록, 단위, 색상, mock 범위 설정
- 포크 개발자가 가장 먼저 수정하는 영역이다.

필드 순서는 펌웨어의 CSV 전송 순서와 같아야 한다. 단순 센서 교체라면 대부분 이 파일과 펌웨어만 바꾸면 된다.
...
`sensor-config.js`만 바꿔도 기본 UI는 자동으로 변한다. 프로젝트 주제에 맞춘 설명, 색상, 레이아웃, 추가 경고나 계산이 필요하면 이 계층을 수정한다.
